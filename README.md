...
**국가 Risk Index 정합성 검증** — 2025년 국가 RI(1~5)와 단기수출보험 국별 사고율·손해율·실질손해율·국가등급·미화국별총위험량(단기)의 **정합성**을 검토하는 단기 분석 PoC.
...
**결과 해석:** [`docs/results-guide.md`](docs/results-guide.md) · **보고서 문장:** [`docs/report_template.md`](docs/report_template.md)
...
| | |
|--|--|
| **In** | 2025 국가 RI ↔ 2025 위험지표 정합성 (비모수·발생여부·민감도·오프라인 HTML·Markdown 보고서) |
| **Out** | ML/예측, 대시보드, 국가×업종 직접 검증, “독립 예측력” 주장 |
...
| 검증 | `input/target/…단기수출보험만.xlsx`, 시트 `단기수출보험`, 헤더 Excel 3행, **F:K** (K=미화국별총위험량(단기), 핵심 타깃) |
...
docs/       plan · results-guide · report_template
...
산출: `outputs/tables/…`, `figures/*.png`, `report/risk_index_validation_report.html` 및 `.md`

고객 설명 본문은 실행 후 HTML(또는 Markdown)이다. 문장 수정은 [`docs/report_template.md`](docs/report_template.md)에서 한다.
